...
import { Download, Loader2, ChevronRight, Smartphone, Square, Circle } from "lucide-react";
...
export default function Home() {
  ...
  }, [username, selectedTheme, device, showStats, shape]);

  // Keep the latest generate() and username in refs so the auto-regenerate
  // effect can fire on appearance changes without also firing on every
  // keystroke in the username field.
  ...
  // When a username is present, changing theme/device/stats updates the
  // preview live (debounced) instead of waiting for another Generate click.
  useEffect(() => {
    if (!usernameRef.current.trim()) return;
    const id = setTimeout(() => generateRef.current(), 200);
    return () => clearTimeout(id);
  }, [selectedTheme, device, showStats, shape]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-14 py-12">
          ...
          <div className="flex justify-center lg:sticky lg:top-8 lg:self-start">
            <div className="w-[240px] h-[520px] border border-white/[0.1] bg-[#0d1117] relative overflow-hidden rounded-[2.2rem] shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
              ...
              {previewSrc && (
                // Preview is a runtime object URL from the canvas blob, which
                // next/image cannot optimize, so a plain <img> is intentional.
